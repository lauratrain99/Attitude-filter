...
% Prior estimates
kf.xi = [qua, imu.gb_dyn]';  % Error vector state
kf.Pi = diag([imu.ini_align_err, imu.ini_align_error(3), imu.gb_dyn].^2);

kf.Q  = diag([imu.arw, imu.g_std, imu.gb_psd].^2);
...
kf.H = [skew_gn O;
        O  skew_mn];
kf.v = diag([imu.a_std, imu.g_std]).^2;
kf.V = [-DCMnb O
        O      DCMnb];
